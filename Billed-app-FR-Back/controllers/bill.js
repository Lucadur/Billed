...
const create = async (req, res) => {
  ...
  try {
    const { name, type, email, date, vat, pct, commentary, status, commentAdmin, amount } = req.body;
    const { file } = req;
...
const get = async (req, res) => {
  ...
  try {
    ...
    const { key: id, name, type, email, date, vat, pct, commentary, status, commentAdmin, fileName, amount, filePath } = bill;
    return res.json({
      id,
      name,
      type,
...
const list = async (req, res) => {
  ...
  if (!user) return res.status(401).send({ message: "user must be authenticated" });
  try {
    const bills = user.type === "Admin" ? await Bill.findAll() : await Bill.findAll({ where: { email: user.email } });
    return res.json(
      bills.map(({ key: id, name, type, email, date, vat, pct, commentary, status, commentAdmin, fileName, amount, filePath }) => ({
        id,
        name,
        type,
        email,
