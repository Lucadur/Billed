...
describe("Test the root path", () => {
  ...
  test("It should not authenticate a user with wrong password", () => {
    return request(app).post("/auth/login").send({ email: "john-doe@domain.tld", password: "mot-de" }).set("Accept", "application/json").expect(400);
  });
  ...
  test("It should loggout a user", () => {
    return request(app).patch("/auth/loggout").set("Authorization", `Bearer ${jwtValue}`).set("Accept", "application/json").expect(200);
  });
});
